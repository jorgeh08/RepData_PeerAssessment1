...
data_NA$Date_POSIXlt <- strptime(data_NA$date, format="%Y-%m-%d")
data_NA$Weekday <- weekdays(data_NA$Date_POSIXlt)
data_NA$Weekday <- factor(data_NA$Weekday, levels= c("Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"))
names(data_NA) <- c("Steps_NA","Date_Vector","Interval","Date_POSIXlt","Weekday")
data_NO_NA <- data_NA[complete.cases(data_NA$Steps_NA),]
```
...
As any data set, sometimes there are missing values (NAs). In this particular dataset, there is a total of `r length(which(is.na(data_NA$Steps_NA)))` missing values.  
